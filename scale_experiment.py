...
init_file = "data_large/Auburn71.nw_init"
dynamic_file = 'data_large/Auburn71.nw_dynamic'
...
def clean_lists(lists, val='\t'):
    final_list = []
    for l in lists:
        final_list.append([x.replace(val,',') for x in l][1:])
    return final_list
...
g_init = clean_lists([g_init],val=" ")[0]
dynamic_splits = clean_lists(dynamic_splits, val=" ")
